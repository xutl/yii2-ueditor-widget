...
class UEditorAction extends Action
{
    ...
    public function init()
    {
        ...
        //预处理
        $this->imageAllowFiles = explode(',', $this->imageAllowFiles);//转数组
        $this->ueditorImageAllowFiles = $this->normalizeExtension($this->imageAllowFiles);
        $this->videoAllowFiles = explode(',', $this->videoAllowFiles);//转数组
        $this->ueditorVideoAllowFiles = $this->normalizeExtension($this->videoAllowFiles);
        $this->fileAllowFiles = explode(',', $this->fileAllowFiles);//转数组
        $this->ueditorFileAllowFiles = $this->normalizeExtension($this->fileAllowFiles);//转数组
        $this->options = ArrayHelper::merge([
            ...
            /* 上传大小限制，单位B */
            "scrawlMaxSize" => $this->imageMaxSize,
            /* 图片访问路径前缀 */
            "scrawlUrlPrefix" => "",
            ...
            "catcherUrlPrefix" => "",
            /* 上传大小限制，单位B */
            "catcherMaxSize" => $this->imageMaxSize,
            /* 抓取图片格式显示 */
    ...
    protected function upload($action)
    {
        switch ($action) {
            ...
            case 'uploadvideo':
                ...
                $config = [
                    'maxFiles' => 1,
                    'extensions' => $this->videoAllowFiles,
    ...
    protected function uploadScrawl()
    {
        ...
        $size = strlen($img);
        $tempName = $this->tempPath . DIRECTORY_SEPARATOR . $size . '.png';
        ...
        return [
            'state' => Yii::t('app', 'File write failed.'),
        ];
    }
    ...
    {
        //查询实例
        $query = FileModel::find()->where(['created_by' => Yii::$app->user->id])->orderBy(['id' => SORT_DESC]);
        /* 判断类型 */
        switch ($action) {
            /* 列出文件 */
            case 'listfile':
                $query->andWhere(['ext' => $this->fileAllowFiles]);
                break;
            /* 列出图片 */
            case 'listimage':
            default:
                $query->andWhere(['ext' => $this->imageAllowFiles]);
        }
        ...
        if ($total > 0) {
            ...
            foreach ($files as $file) {
                array_push($lists, [
                    'original' => $file['name'],
                    'url' => $this->controller->module->getStorageUrl() . '/' . $file['path'],
    ...
    private function getRemoteImage($imgUrl)
    {
        ...
        if (strpos($imgUrl, "http") !== 0) {
            return [
                'state' => Yii::t('app', 'Link is not a HTTP link.'),
            ];
        }
        //获取请求头并检测死链
        ...
        if (!(stristr($heads[0], "200") && stristr($heads[0], "OK"))) {
            return [
                'state' => Yii::t('app', 'Link unavailable.'),
            ];
        }

        //格式验证(扩展名验证和Content-Type验证)
        $fileType = strtolower(pathinfo($imgUrl, PATHINFO_EXTENSION));
        if (!in_array($fileType, $this->imageAllowFiles) || !stristr($heads['Content-Type'], "image")) {
            return [
                'state' => Yii::t('app', 'Link contentType not correct.'),
            ];
        }
    ...
    private function saveFile(FileObject $file)
    {
        ...
        if (file_exists($file->tempName)) {
            ...
            $result['state'] = Yii::t('app','File does not exist.');//文件不存在
        }
        return $result;
    }
}